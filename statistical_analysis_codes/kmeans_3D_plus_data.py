...
#                                Gerando k-means
#------------------------------------------------------------------------------------
def cluster_analysis_3d_plus_data(df, n_c, axis_labels=['x', 'y', 'z'], plus_data=[], data_labels=[], name=''):
    ...
    marker_list = ['s','x','^','*','D','p','>','H']
    colors_list = ['k', 'b', 'g', 'm', 'c', 'y', 'r']
    ...
    for cluster in list(np.arange(1,n_c+1)):
        ...
        #------------------------------------------------------------------------------------
        for d_indx in range(len(plus_data)):

            data_array = plus_data[d_indx]
            data_lab = data_labels[d_indx]
            marker_i = marker_list[d_indx]
            color_i = colors_list[d_indx]
            
            x_data = data_array[0]
            y_data = data_array[1]
            z_data = data_array[2]
            
            ax0.scatter(x_data, y_data, z_data, s=45, c=color_i, marker = marker_i, label=data_lab+':'+'\n'+\
                    str(round(x_data,2))+' , '+str(round(y_data,2))+', '+str(round(z_data,2)))
            ax1.scatter(x_data, z_data, s=45, c=color_i, marker = marker_i) # x, z
            ax2.scatter(y_data, z_data, s=45, c=color_i, marker = marker_i) # y, z
            ax3.scatter(x_data, y_data, s=45, c=color_i, marker = marker_i) # x, y
