...
#                                Gerando k-means
#------------------------------------------------------------------------------------
def cluster_analysis_2d_flags(df, flags, n_c,  axis_labels=['x', 'y'], name=''):

    # APAGAR
    #save_groups=1
    #df = params_frame[['Country', 'Skewness_2', 'Alfa']]
    #n_c = 8
    #axis_labels=['x', 'y']
    #name=''
    #
    ...
    #------------------------------------------------------------------------------------
    # Salvo cada ponto no espaço de parâmetros de acordo
    # com seu label de entrada e os labels dados pelo kmeans
    # Melhor numero de cluster
    n_c_best = visualizer.elbow_value_
    # Recalculo kmeans para este valor
    kmeans = KMeans(n_clusters = n_c_best).fit(df)
    # Recupero os labels
    cluster_labels = kmeans.labels_
    cluster_xy = kmeans.cluster_centers_
    # Salvo em colunas
    for label in list(np.unique(cluster_labels)):
        flag_list = []
        for indx in np.argwhere(cluster_labels==label).T[0]:
            # Nome, x, y
            flag_list.append([flags[indx], df.values[indx][0], df.values[indx][1]])
        # Exportando flags
        file_name = 'cluster_' + str(label) + '_x_' + str(round(cluster_xy[label][0],2)) + \
                    '_y_' + str(round(cluster_xy[label][1],2))
        #
        flag_frame = pd.DataFrame(flag_list, columns = ['ID', axis_labels[0], axis_labels[1]])
        flag_frame.to_csv(name+'_'+file_name+'_flags.csv', index=False)
    #====================================================================================
    #                                Exportando Dados
    #------------------------------------------------------------------------------------
